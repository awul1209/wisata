        ...
        if (!routingControl) {
            ...
            routingControl.on('routesfound', function(e) {
                ...
                route.instructions.forEach(function(step) {
                    let text = step.text.replace("Turn left", "Belok kiri").replace("Turn right", "Belok kanan").replace("Continue straight", "Lurus terus").replace("Continue", "Lanjutkan").replace("Destination", "Tujuan").replace("at the roundabout", "di bundaran").replace("Take the", "Ambil").replace("exit", "jalan keluar").replace(" to stay on", " untuk tetap di").replace("onto", "menuju").replace("Head", "Menuju").replace("You have arrived at your destination, on the right", "Anda telah tiba di tujuan Anda, di sebelah kanan.").replace("Exit the traffic circle", "Keluar dari bundaran lalu lintas").replace("Enter the traffic circle and take the 1st", "Masuk ke bundaran lalu ambil jalan pertama.").replace("west on", "ke barat").replace("east", "timur").replace("west", "barat").replace("northeast", "timur laut").replace("southwest ", "barat daya");
